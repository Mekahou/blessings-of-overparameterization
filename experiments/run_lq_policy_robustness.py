...
if __name__ == "__main__":
    ...
    # First Experiment: LeakyRelu
    results_policy_1_layer_leakyrelu = []
    for h in grid_dim_hidden:
        for seed in range(50):
            ...
            results_policy_1_layer_leakyrelu.append({
                'dim_hidden'                            : h,
                'seed'                                  : seed,
            ...
            pd.DataFrame(results_policy_1_layer_leakyrelu).to_csv(
                'results/lq/LQ_results_policy_1_layer_leakyrelu.csv', index=False
            )
    # Second Experiment: Sigmoid
    results_policy_1_layer_sigmoid = []
    for h in grid_dim_hidden:
        for seed in range(50):
            ...
            results_policy_1_layer_sigmoid.append({
                'dim_hidden'                            : h,
                'seed'                                  : seed,
                'param_num_u'                           : out['param_num'],
                'train_loss_euler'                      : out['train_loss_euler'],
            ...
            pd.DataFrame(results_policy_1_layer_sigmoid).to_csv(
            'results/lq/LQ_results_policy_1_layer_sigmoid.csv', index=False
            )
