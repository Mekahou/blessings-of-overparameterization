...
if __name__ == "__main__":
    ...
    # First Robustness Experiment: 2 hidden layers, ReLU
    ...
    for h in grid_dim_hidden:
        for seed in range(50):
            ...
            # Save after every run — guards against crash during long experiment
            pd.DataFrame(results_2_layer_relu).to_csv(
                "results/mccall/McCall_results_2_layer_relu.csv", index=False
            )

    # Second Robustness Experiment: 2 hidden layers, LeakyReLU
    ...
    for h in grid_dim_hidden:
        for seed in range(50):
            ...
            pd.DataFrame(results_2_layer_leakyrelu).to_csv(
                "results/mccall/McCall_results_2_layer_leakyrelu.csv", index=False
            )
